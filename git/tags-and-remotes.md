# Tags vs remotes
In git, there is no linkt (in `.git/refs/` dir or `.git/packed-refs` file) that
...
In contrast, this is different than branches, where locak `refs` DB has 
explicit info about remote tags (its unclear to me how "tracking" branches
